...
void validate_button5_clicked(GtkWidget *widget, gpointer userData_option5){
    UserData_option5 *data = (UserData_option5 *)userData_option5;

    GtkWidget *resultWindow5 = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(resultWindow5), "Result Window");
    gtk_window_set_default_size(GTK_WINDOW(resultWindow5), 750, 650);
    gtk_window_set_position(GTK_WINDOW(resultWindow5), GTK_WIN_POS_CENTER);

    // Create a ListBox to display the results
    GtkWidget *listbox = gtk_list_box_new();

    // Retrieve the entered region from the entry buffer
    const char *selected_option = gtk_combo_box_text_get_active_text((GtkComboBoxText *) data->combobox);

    printf("selected option : %s",selected_option);

    FILE *f = NULL;
    struct Tenrg e;
    int s = 1024;
    Ouvrire(&f, selected_option, 'A');
...
// Set the scrolled window as the content of the main window
gtk_container_add(GTK_CONTAINER(resultWindow5), scrolledWindow);
// Show all widgets in the new window
gtk_widget_show_all(resultWindow5);

Fermer(f);

}

void option5_clicked(GtkWidget *widget, gpointer data){

    // Create a new window
    GtkWidget *buttonWindow5 = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_default_size(GTK_WINDOW(buttonWindow5), 300, 100);
    gtk_window_set_position(GTK_WINDOW(buttonWindow5), GTK_WIN_POS_CENTER);

    // Create a box to hold the content in the new window
    GtkWidget *buttonBox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    gtk_container_add(GTK_CONTAINER(buttonWindow5), buttonBox);


    GtkWidget *buttonLabelWidget2 = gtk_label_new(" Selectionner le fichier à afficher son contenu ");
    gtk_box_pack_start(GTK_BOX(buttonBox), buttonLabelWidget2, FALSE, FALSE, 10);

    // Create a combo box for region selection
    GtkWidget *regionComboBox = gtk_combo_box_text_new();
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(1RM-Blida)");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(2RM-Oran)");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(3RM-Bechar)");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(4RM-Ouargla)");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(5RM-Constantine)");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(regionComboBox), "PERSONNEL-ANP_DZ.dat(6RM-Tamanrasset)");
    ...
    // Create user data structure and set its values
    UserData_option5 *userData = g_new(UserData_option5, 1);
    userData->mainWindow = buttonWindow5;
    userData->combobox = regionComboBox;

    // Connect the callback function to the "clicked" signal of the "Valider" button
    g_signal_connect(validerButton, "clicked", G_CALLBACK(validate_button5_clicked), userData);

    // Pack widgets into the box
    gtk_box_pack_start(GTK_BOX(buttonBox), validerButton, FALSE, FALSE, 5);

    // Show all widgets in the new window
    gtk_widget_show_all(buttonWindow5);
}
    ...
    if (strcmp(buttonLabel, "Chargement initiale") == 0) {
        ...
        for (int i = 0; i < G_N_ELEMENTS(option_labels); ++i) {
            ...
            if (i == 0) {
                ...
                g_signal_connect(option_button, "clicked", G_CALLBACK(option5_clicked), popover);
            }
        }
